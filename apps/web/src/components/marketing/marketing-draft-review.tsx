...
import { useMarketingDrafts, useSendDraft } from '@/hooks/use-marketing';
...
// text, deletes the stale draft and marks the row SENT. All data is REAL (via
// /marketing/drafts + /marketing/drafts/send) — no mock content.
...
export function MarketingDraftReview() {
  ...
  // `key` on the inputs (below) resets them whenever the selected draft changes.
  ...
  if (drafts.data && !drafts.data.configured) {
    return (
      <Card className="p-6 text-sm text-muted-foreground">
        Draft Review isn&rsquo;t connected yet — set <code>N8N_API_URL</code> on
        the API so it can read the RAG Agent workflow.
      </Card>
    );
  }
  ...
  if (list.length === 0) {
    return (
      <Card className="flex flex-col items-center justify-center gap-2 p-10 text-center text-sm text-muted-foreground">
        ...
        No drafts awaiting review. The RAG Agent posts a draft here whenever it
        replies to an &ldquo;Unsure&rdquo; lead.
      </Card>
    );
  }
      ...
      <Card className="p-5">
        ...
        {!d.sendable ? (
          <div className="mt-2.5 rounded-lg border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-600 dark:text-amber-400">
            ⚠ This draft predates draft-id capture, so it can&rsquo;t be sent from
            the ERP. Newer drafts carry a Gmail draft id and are sendable.
          </div>
        ) : null}
