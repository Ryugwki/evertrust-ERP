...
### Phase 7 — Documents + QC + submit (R32–R37) — 🟡 R34–R37 DONE (2026-05-31)
- [ ] **R32–R33 — TYPE 2 doc prep + completeness** — FOLLOW-ON. Master checklist comes from the external "Library → EPC Document" (the required-forms manifest); `doc_packages`/`compliance_checks` tables exist but missing-form detection isn't deterministic until that source is defined. TYPE 2 docs already upload via the Phase 4 documents module.
- [x] **R34 — conditional QC gate** — pure `qcRequired` predicate: QC required when the tender is above the EU threshold (high-value) **OR** pricing is high-risk (≥35% unbacked / top-5 unbacked, via `computeTenderRisk`) **OR** a QC review was explicitly opened. Routine tenders skip QC. The QC review reuses `approval_type 'QC'` (request = `tenders:write`, decide = `approvals:decide`).
- [x] **R35–R37 — submit act + evidence** — `POST /tenders/:id/submit` (proof + auto file-list snapshot) enforces ALL `submissionBlockers` (customer approval + conditional QC + must be DOCUMENTS), writes a `submission_receipt` (immutable; `file_list` jsonb added in migration `0008`) and advances DOCUMENTS→SUBMITTED. The generic transition now REFUSES a direct →SUBMITTED, so **SUBMITTED ⟺ a logged receipt** (no submission without evidence). `GET /tenders/:id/submission` = the readiness/gate state the web Submission card renders (same shared predicates). Portal act stays human. Submit gated `tenders:transition`.
- Verify: tsc shared/db/api/web clean; +qcRequired/submissionBlockers + submission.service + submission.permissions specs; **205 api tests green**. ⏳ **Migration `0008` deploy PENDING** (Docker daemon was down at build time) — rebuild image + migrate (see changelog).
...
- **2026-05-31** — **Phase 7 R34–R37 done** (not yet committed): conditional QC gate + human submit act + evidence logging. New `SubmissionModule` — `GET /tenders/:id/submission` (readiness) + `POST /tenders/:id/submit` — enforces the shared `submissionBlockers` (Phase 6 customer approval + Phase 7 `qcRequired`), writes an immutable `submission_receipt` (proof + `file_list` snapshot, migration `0008_submission_file_list`) and advances to SUBMITTED; the generic `transition()` now refuses a direct →SUBMITTED (SUBMITTED ⟺ a receipt). Web Submission card (readiness + QC request/decide + record-submission dialog + receipts); SUBMITTED removed from the generic transition buttons. **205 api tests green; tsc shared/db/api/web clean.** ⏳ **Deploy pending:** Docker daemon was down — when up, run `docker compose -f infra/docker-compose.yml up -d --no-deps --build api` (rebuilds the shared image with `0008`) then `docker compose -f infra/docker-compose.yml run --rm --no-deps migrate sh -c "corepack pnpm --filter @evertrust/db db:migrate"`. R32–R33 (TYPE 2 master-checklist completeness) is the follow-on (needs the EPC Library). **Next: Phase 2/3 intake (Argus/Scribe/Sieve).**
